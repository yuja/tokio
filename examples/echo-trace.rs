...
fn main() {
    ...
    tokio_trace::collect::with_default(&trace, || {
        ...
        // current thread until all tasks complete execution. Since the `done`
        // task never completes (it just keeps accepting sockets), `tokio::run`
        // blocks forever (until ctrl-c is pressed).
